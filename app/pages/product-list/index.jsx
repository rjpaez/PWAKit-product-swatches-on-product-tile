...
ProductList.getProps = async ({ res, params, location, api }) => {
    ...
    const [category, productSearchResult] = await Promise.all([
        isSearch ? Promise.resolve() : api.shopperProducts.getCategory({ parameters: { id: categoryId, levels: 0 } }),
    ...
    productSearchResult.hits.map((hit, index) => {
        productSearchResult.hits[index].shopperProduct = shopperProducts[index]
    });
...
ProductList.propTypes = {
    ...
    shopperProducts: PropTypes.object
}

export default ProductList

/**
 * this function fetches additional product data for a given product search result using 
 * the api.shopperProducts.getProducts() method. product IDs are split into chunks, with each 
 * chunk consisting 24 product IDs. this data is fetched in parallel using the Promise.all() 
 * method. the result of Promise.all() is stored as an array for each chunk of product IDs and
 * combined into a single shopperProducts array which is returned as the result of the function.
 * 
 * @param   {Object}    productSearchResult : product search result object
 * @param   {Object}    api                 : api object
 * @returns {Array}     shopperProducts     : array of product data returned from api
 */
const fetchShopperProducts = async (productSearchResult, api) => {
    const productIds = productSearchResult.hits.map((item) => item.productId)
    const chunkSize = 24
    const shopperProducts = []

    const chunks = productIds.reduce((resultArray, item, index) => {
    ...
    const results = await Promise.all(chunks.map(async chunk => {
        const tempData = await api.shopperProducts.getProducts({
            parameters: {
                ids: chunk.join(','),
                allImages: true // required to generate all swatch images
            }
        })
        return tempData.data.map(item => item)
    }))

    shopperProducts.push(...results.flat())
